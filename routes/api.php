...
// =======================
Route::prefix('display')->name('api.display.')->group(function () {
...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::prefix('super-admin')
        ...
        ->group(function () {
            ...
            // ✅ DASHBOARD STATS (INI YANG TADI HILANG)
            ...
            Route::patch(
                'organizations/{organization:slug}/toggle-status',
                [OrganizationController::class, 'toggleStatus']
            )->name('organizations.toggle-status');

            // Organization Admins
            Route::get(
                'organizations/{organization:slug}/admins',
                [OrganizationController::class, 'getAdmins']
            )->name('organizations.admins.index');

            Route::post(
                'organizations/{organization:slug}/admins',
                [OrganizationController::class, 'addAdmin']
            )->name('organizations.admins.store');

            Route::delete(
                'organizations/{organization:slug}/admins/{user}',
                [OrganizationController::class, 'removeAdmin']
            )->name('organizations.admins.destroy');

            Route::post(
                'organizations/{organization:slug}/admins/{user}/reset-password',
                [OrganizationController::class, 'resetAdminPassword']
            )->name('organizations.admins.reset-password');
        });
